...
const Press = use('App/Services/Press')
...
class AppController {
  ...
  async fixOrders ({ user, params }) {
    ...
    const items = await DB('order_item as oi')
      .select('customer.*', 'user.email', 'user.lang', 'oi.id', 'oi.quantity', 'oi.order_shop_id', 'oi.project_id',
        'vod.barcode', 'os.transporter', 'os.date_export', 'os.whiplash_id', 'os.whiplash_id2')
      ...
      .whereIn('oi.order_shop_id', shops.map(s => s.order_shop_id))
      .all()

    const dau = await DB('daudin')
      .select('*')
      .where('id', 1274)
      .first()
    ...
    for (const order of Object.values(orders)) {
      if (order.transporter === 'daudin' && order.date_export) {
        ...
        for (const i in order.items) {
          ...
          order.items[i].found = lines.findIndex(ii => ii.barcode === order.items[i].barcode) !== -1
      ...
      if (order.transporter === 'whiplash' && order.whiplash_id && !order.whiplash_id2) {
        ...
        **/
        ...
        for (const i in order.items) {
          order.items[i].found = o.order_items.findIndex(ii => ii.sku === order.items[i].barcode) !== -1
          if (!order.items[i].found) {
  ...
  async subscribeNewsletterWithoutAccount ({ params }) {
    ...
    const exists = await DB('newsletter_no_account')
    ...
    if (!exists) {
      // Insert in db
      const [id] = await DB('newsletter_no_account')
        .insert({
          email: params.email,
