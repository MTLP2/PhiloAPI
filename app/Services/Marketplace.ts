...
class Marketplace {
  static getItems = (userId) => {
    return DB()
      ...
      .leftJoin('order_shop as os', function () {
        this.on('os.shop_id', '=', 'm.id').on('os.step', '=', DB.raw("'pending'"))
      })
  ...
  static saveItem = async (params) => {
    ...
    item.release_id =
      params.album && params.album.typee === 'release' ? params.album.discogs_id : null
    item.master_id =
      params.album && params.album.typee === 'master' ? params.album.discogs_id : null
    ...
    await DB('marketplace').where('user_id', params.user_id).update({
      active: 1,
      first_reference: 1,
  ...
  static removeItem = async (params) => {
    ...
    if (item.user_id !== params.user_id) {
      ...
      await DB('marketplace_item').where('id', params.id).update({ quantity: 0 })
    }
    return true
  }

  static search = async (artist, title) => {
    if (artist.length < 3) {
      return []
    }
  ...
  static getShippingCosts = async (params) => {
    ...
    const shipping = await DB('shipping_costs').where('user_id', params.user.user_id).all()
  ...
  /**
  static getShippingCostsOld = async (params) => {
    const resp = {}
  ...
  **/
  static saveShippingCosts = async (params) => {
    ...
    await DB('marketplace').where('user_id', params.user.user_id).update({
      shipping_costs: 1
    })
  ...
  static checkActive = async (id, update) => {
    const market = await DB('marketplace').where('user_id', id).first()
    ...
    if (update) {
      await DB('marketplace').where('user_id', id).update({
        active: active,
        updated_at: Utils.date()
      })
    }
  ...
  /**
  static saveShippingCostsOld = async (params) => {
  ...
  **/
  ...
  static getItemsByProject = async (id, userId) => {
    ...
    const user = await DB('user').where('id', userId).belongsTo('customer').first()
  ...
  static getMarketplace = async (params) => {
    const marketplace = await DB('marketplace')
  ...
  /**
  static saveSettings = async (params) => {
  ...
  **/
  ...
  static saveInformation = async (params) => {
    try {
      const marketplace = await DB('marketplace').where('user_id', params.user_id).first()

      const user = await DB('user').select('ip').where('id', params.user_id).first()

      if (!marketplace) {
        await DB('marketplace').insert({
          user_id: params.user_id,
          created_at: Utils.date(),
          updated_at: Utils.date()
        })
  ...
  static saveBankAccount = async (params) => {
    const marketplace = await DB('marketplace')
      .where('user_id', params.user_id)
      .belongsTo('customer')
      .first()
    ...
    if (params.remove_account) {
      await stripe.accounts.deleteExternalAccount(marketplace.stripe_account, params.remove_account)
      return true
    }
  ...
  static exportSales = async (params, res) => {
    params.shop_id = params.user_id
    params.user_id = null
  ...
  static setSaleAffiliation = (id, type, comment) => {
    if (type === 'refused') {
      ...
      if (orderShop.step === 'pending' && params.status === 'confirmed') {
        ...
        items.map(async (item) => {
          if (response[item.id]) {
            ...
            refund += item.total
            await DB('order_item').where('id', item.id).update({
              refused: 1
            })
          }
        })
  ...
  static prepareImport = async (params) => {
    let columns = []
    const lines = []
    ...
    if (ext === 'csv') {
      const data = Buffer.from(
        params.file.data.replace(/^data:[\w]+\/[\w.-]+;base64,/, ''),
        'base64'
      )
        .toString()
        .split('\r\n')
  ...
  static getLinesImport = (file, col) => {
    const refs = {}
    ...
    file.lines.map((line) => {
      ...
      if (
        (row.condition && row.condition.toLowerCase() === 'neuf') ||
  ...
  static previewImport = async (params) => {
  ...
  static startImport = async (params) => {
    const file = await Marketplace.prepareImport(params)
    const rows = Marketplace.getLinesImport(file, params.columns)
        ...
        rows.map(async (row) => {
          await DB('marketplace_import').insert({
            ...
            price_wholesale: row.price_wholesale
              ? row.price_wholesale.toString().replace(',', '.').replace(/"/g, '').substring(0, 20)
              : null,
            label: row.label ? row.label.toString().substring(0, 200) : null,
            sleeve_condition: row.sleeve_condition
              ? row.sleeve_condition.toString().substring(0, 200)
              : null,
            media_condition: row.media_condition
              ? row.media_condition.toString().substring(0, 200)
              : null,
  ...
  static getImports = async (params) => {
  ...
  static convertImport = async () => {
    ...
    await Utils.sequence(
      lines.map((line, l) => async () => {
        ...
        const item = await DB('marketplace_import').where('id', line.id).first()

        if (item.status !== 'pending') return false
        else {
          await DB('marketplace_import').where('id', line.id).update({
            status: 'in_progress',
            updated_at: Utils.date()
          })
        }
        ...
        if (projectId) {
          ...
          if (existing) {
            ...
            const result = await DB('marketplace_item').insert({
    ...
    imports.map(async (imp) => {
      ...
      **/
      ...
      await DB('marketplace_imports').where('id', imp.id).update({
        status: 'finished',
        updated_at: Utils.date()
      })
      await Notification.new({
  ...
  **/
  ...
  static calculateShipping = async (params) => {
    ...
    const country = await DB('country').where('id', params.country_id).first()
  ...
  static export = async (params, res) => {
    const items = await DB()
      .select(
        'p.name',
        'p.artist_name',
