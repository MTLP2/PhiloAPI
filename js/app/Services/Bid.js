...
const Stripe = use('App/Services/Stripe')
...
class Bid {
  static async find (id, params = {}) {
    ...
    const selects = ['bid.id', 'bid.status', 'bid.user_id', 'bid.total', 'bid.currency', 'bid.created_at', 'user.name']

    if (params.for === 'sheraf') {
      selects.push(...['order.user_agent', 'bid.invoice_id', 'bid.customer_id', 'user.email', 'user.country_id', 'order.payment_id'])
    }
    let bids = DB('bid')
      .select(...selects)
  ...
  static async pay (params) {
    ...
    if (params.card.customer) {
      ...
      const customer = await Stripe.getCustomer(params.user.id)
  ...
  static async payConfirm (params) {
    if (params.payment_intent_id) {
      const project = await Bid.find(params.id)
      const order = await DB('order')
        .where('payment_id', params.payment_intent_id)
        .first()
  ...
  static async confirmCapture ({ bid, order }) {
    ...
    for (const b of oldBids) {
      ...
      if (b.user_id === bid.user_id) {
        ...
        await Notification.add({
          ...
          bid_id: b.id,
          order_id: b.order_id,
          data: `${b.total} ${Antl.formatMessage(`base.${b.currency}`)}`
        })
      }
    }
    ...
    await Notification.add({
      ...
      data: `${bid.total} ${Antl.formatMessage(`base.${bid.currency}`)}`
    })
  ...
  static async cancel (params) {
    ...
    if (params.user.id !== bid.user_id && !await Utils.isTeam(params.user.id, 'boss')) {
