...
class ProjectEdit {
  ...
  **/
  ...
  static setDurationByProject = async (id) => {
  ...
  static getDiscogsReference = async (id, type) => {
    const db = new Discogs({
      consumerKey: global.cron ? config.discogsCron.consumer_key : config.discogs.consumer_key,
      consumerSecret: global.cron
        ? config.discogsCron.consumer_secret
  ...
  static existDiscogs = async (album) => {
    let masterId = null
    let releaseId = null
    ...
    let found = DB('project').select('id')
  ...
  static createApiProject = async (params, album) => {
    if (!album) {
    ...
    await Promise.all(
      project.tracklist.map(async (track) => {
        await DB('song').save({
          project_id: projectId,
          title: track.title,
          artist: track.artists
            ? track.artists.map((a) => a.name.replace(/ *\([\d]*\) */g, '')).join(', ')
            : null,
  ...
  static saveReference = async (params) => {
    let p = {}
    if (params.id) {
    ...
    await DB('song').where('project_id', p.id).whereNotIn('id', tracks).delete()

    if (params.styles.length > 0) {
      params.styles.map(async (style) => {
        const s = await DB('project_style')
          .where({
            project_id: p.id,
            style_id: style.id
          })
