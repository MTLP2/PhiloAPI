...
User.me = (id) => {
  ...
  return Promise.all([user, notifications, alerts])
    .then((data) => {
      const u = data[0]
...
User.find = async (params) => {
  ...
  **/
  user.followers = await DB('user').select('id', 'name', 'slug')
    .join('follower', 'follower.follower', 'user.id')
    .where('follower.user_id', user.id)
    .all()
  user.following = await DB('user').select('id', 'name', 'slug')
...
User.convertPassword = (password) =>
  bcrypt.hashSync(password, 1).replace('$2a$', '$2y$')

User.updateIp = async (id, ip) => {
  return DB('user').where('id', id).update({ ip })
}

User.updateProfile = async (userId, params) => {
...
User.existsByEmail = async (email) => {
...
User.updateCurrency = (userId, params) => {
  cio.identify(userId, {
    currency: params.currency
  })
...
User.updatePassword = async (userId, params) => {
  const user = await DB('user')
    .where('id', userId)
    .first()
...
User.updatePictureFromUrl = (userId, url, social) => {
...
User.updatePicture = (id, buffer, social) => {
  return new Promise(async (resolve, reject) => {
    ...
    Storage.deleteFolder(`profiles/${
      user.picture !== '1' && user.picture !== '0'
        ? user.picture
        : user.id
    }`)
...
User.updateDelivery = async (userId, params) => {
  const data = params
  const user = await DB('user')
    .where('id', userId)
    .first()
...
User.updateNotifications = (userId, params) => {
  DB('user')
    .where('id', userId)
    .update({
...
User.setNotificationsView = (userId) =>
  DB('notification')
    .where('user_id', userId)
    .where('alert', 1)
    .where('new', 1)
...
User.getMessagesByUser = async (userId, from) => {
  const query = `
    SELECT m.*, DATE_FORMAT(m.created_at,'%Y-%m-%d %H:%i') as created_at,
      u1.name AS from_name, u1.slug, u2.name AS to_name
    FROM message m
...
User.sendMessage = (userId, params) =>
  DB('message')
    .insert({
      from: userId,
      to: params.to,
...
User.getBoxes = async (params) => {
  ...
  for (let i = 0; i < boxes.length; i++) {
    ...
    for (const dispatch of dispatchs) {
      ...
      if (!dispatch.date_export && moment(dispatch.created_at).format('YYYY-MM') === moment().format('YYYY-MM')) {
        box.dispatch_left++
      }
...
User.getProjectOrders = async (params) => {
  await Utils.checkProjectOwner({ project_id: params.id, user: params.user })

  const orders = await DB('order_item as oi')
    .select('oi.id', 'oi.price', 'os.shipping', 'os.sub_total', 'tips',
      'oi.quantity', 'os.tax', 'os.created_at', 'os.currency',
      'os.total', 'os.user_id', 'user.name as user_name', 'os.customer_id', 'os.customer_invoice_id')
...
/**
User.extractProjectOrders = async (params) => {
  await Utils.checkProjectOwner({ project_id: params.id, user: params.user })
...
User.unsubscribeNewsletter = async (params) => {
...
User.getFullData = async (params = {}) => {
  const currenciesDb = await Utils.getCurrenciesDb()
  const currencies = Utils.getCurrencies('EUR', currenciesDb)

  let users = DB('user')
    .select(
      'id', 'name', 'email', 'customer_id', 'country_id', 'last', 'birthday', 'unsubscribed', 'newsletter',
      'lang', 'styles', 'type', 'user.created_at', 'currency', 'cio_update', 'mailjet_id', 'mailjet_update'
    )
    .whereNotNull('email')
    .belongsTo('customer')
  ...
  for (const user of users) {
    ...
    const projects = await DB('vod')
      .select('project.created_at', 'vod.updated_at', 'com_id', 'vod.start', 'vod.step', 'vod.count')
    ...
    for (const box of boxes) {
      ...
      if (moment() < moment(box.end)) {
        user.box_months = Math.round(moment.duration(moment().diff(moment(box.start))).asMonths()) + 1
      } else {
        user.box_months = Math.round(moment.duration(moment(box.end).diff(moment(box.start))).asMonths()) + 1
      }
...
User.syncCIO = async (user) => {
  const params = {
    ...
    projects_date_launched: user.projects_date_launched ? moment(user.projects_date_launched).unix() : null,
    projects_date_saved: user.projects_date_saved ? moment(user.projects_date_saved).unix() : null,
    boxes: user.boxes,
