...
class Box {
  static all (params) {
    params.query = DB('box')
      .select('box.*',
        'user.email', 'user.name as user_name',
        'c.firstname', 'c.lastname',
        'box_code.partner',
        'user2.email as buy_email',
        DB.raw('(select created_at from box_dispatch where box_id = box.id order by created_at desc limit 1) as last_dispatch')
      )
    ...
    for (const i in filters) {
      if (filters[i] && filters[i].name === 'box.dispatchs') {
        params.query.whereExists(
          DB('box_dispatch')
            .where('box_id', DB.raw('box.id'))
            .where('id', filters[i].value)
            .query()
        )
        filters.splice(i, 1)
  ...
  static async find (id) {
    const box = await DB('box')
      .select('box.*', 'user.email', 'buyer.stripe_customer', 'buyer.id as buyer_id',
        'buyer.email as buyer_email',
        'box_code.id as is_gift', 'box_code.partner')
      .where('box.id', id)
      .leftJoin('user', 'user.id', 'box.user_id')
    ...
    box.records = await DB('box_project')
      .select('box_project.*',
        'p1.name as p1_name', 'p1.artist_name as p1_artist', 'p1.picture as p1_picture', 'p1.slug as p1_slug', 'p1.id as p1',
        'p2.name as p2_name', 'p2.artist_name as p2_artist', 'p2.picture as p2_picture', 'p2.slug as p2_slug', 'p2.id as p2',
        'p3.name as p3_name', 'p3.artist_name as p3_artist', 'p3.picture as p3_picture', 'p3.slug as p3_slug', 'p3.id as p3',
        'p4.name as p4_name', 'p4.artist_name as p4_artist', 'p4.picture as p4_picture', 'p4.slug as p4_slug', 'p4.id as p4',
        'p5.name as p5_name', 'p5.artist_name as p5_artist', 'p5.picture as p5_picture', 'p5.slug as p5_slug', 'p5.id as p5',
        'p6.name as p6_name', 'p6.artist_name as p6_artist', 'p6.picture as p6_picture', 'p6.slug as p6_slug', 'p6.id as p6'
      )
      .where('box_id', box.id)
    ...
    const barcodes = []
    box.dispatch = (await DB('box_dispatch')
      .orderBy('id', 'asc')
      .where('box_id', box.id)
      .all()
    ).map(d => {
  ...
  static async stats (params) {
    const boxes = await DB('box')
      ...
      .leftJoin('order_box', (query) => {
        query.on('order_box.box_id', '=', 'box.id')
          .andOn('order_box.id', '=', DB.raw('(select max(id) from order_box where order_box.box_id = box.id)'))
      })
      .leftJoin('box_code', 'box_code.box_id', 'box.id')
      .all()
  ...
  static async calculate (params) {
    ...
    if (prices.promo) {
      res.discount = prices.prices[params.type][params.periodicity][params.currency] - prices.prices_discount[params.type][params.periodicity][params.currency]
    ...
    **/
    ...
    if (params.promo_code) {
      const promo = await DB('promo_code')
        ...
        .where(function () {
          this.whereNull('type_box')
            .orWhere('type_box', `${params.type}_${params.periodicity}`)
        })
        .where(function () {
          this.where('is_once', 0)
            .orWhereNotExists(
        ...
        .where(function () {
          this.where('unique', 0)
            .orWhereNotExists(
              DB.raw(`SELECT * FROM order_box WHERE promo_code = promo_code.code AND order_box.user_id = '${params.user_id}'`)
            )
        })
        .first()
  ...
  static async setDispatchs () {
    ...
    const usersProjects = await DB()
      ...
      .where('is_paid', true)
      .whereIn('user_id', boxes.map(b => b.user_id))
      .all()
    ...
    for (const p in projects) {
      ...
      project.genres = project.styles.split(',').filter(s => s !== '').map(s => {
        return styles.find(ss => +ss.id === +s).genre_id
      })

      stocks[project.id] = project.stock < project.stock_daudin ? project.stock : project.stock_daudin
    }
    ...
    for (const box of boxes) {
      ...
      const pp = [
        ...Utils.shuffle(projects),
        ...Utils.shuffle(selections)
      ]

      const already = {}
      for (const p of pp) {
        if ((boxDispatchs[box.id] && boxDispatchs[box.id][p.barcode]) || (users[box.user_id] && users[box.user_id].some(pp => pp === p.project_id))) {
        ...
        if (box.styles) {
          ...
          if (intersection.length > 0) {
            for (let i = 1; i <= box.nb_vinyl; i++) {
              if (barcodes.length < box.nb_vinyl && stocks[p.id] > 0 && !barcodes.find(b => b === p.barcode)) {
                stocks[p.id]--
  ...
  **/
  ...
  static async checkPayments (params) {
    ...
    for (const box of boxes) {
      ...
      if (moment(lastPayment.created_at).format('YYYY-MM') !== moment().format('YYYY-MM') && dispatch) {
      ...
      try {
        ...
        if (pay.status === 'succeeded') {
          const txn = await stripe.balanceTransactions.retrieve(pay.charges.data[0].balance_transaction)
  ...
  static async getPrices () {
    ...
    if (sales) {
      ...
      for (const d of data) {
        ...
        res.prices_discount[d.type][d.periodicity].EUR = Utils.round(res.prices[d.type][d.periodicity].EUR - (res.prices[d.type][d.periodicity].EUR * (sales.value / 100)), 0)
        res.prices_discount[d.type][d.periodicity].USD = Utils.round(res.prices[d.type][d.periodicity].USD - (res.prices[d.type][d.periodicity].USD * (sales.value / 100)), 0)
        res.prices_discount[d.type][d.periodicity].GBP = Utils.round(res.prices[d.type][d.periodicity].GBP - (res.prices[d.type][d.periodicity].GBP * (sales.value / 100)), 0)
        res.prices_discount[d.type][d.periodicity].AUD = Utils.round(res.prices[d.type][d.periodicity].AUD - (res.prices[d.type][d.periodicity].AUD * (sales.value / 100)), 0)
      }
    }
  ...
  static async selectVinyl (params) {
    ...
    const sponsor = await DB('box_sponsor')
      .whereNull('used')
      .where('box_id', box.id)
      .all()
  ...
  static async checkStock (month) {
    const projects = await DB('box_month')
      .where('date', month)
      .all()
  ...
  static async confirmCode (params) {
    ...
    box.address_pickup = params.pickup && params.pickup.number ? JSON.stringify(params.pickup) : null
  ...
  static async giftCard (code) {
    ...
    if (code.box_id) {
      ...
      code = await DB('box_code')
        .where('box_id', code.box_id)
        .first()
      code.lang = lang
    }
    const html = View.render('box', {
      ...code,
      lang: code.lang,
      t: v => Antl.forLocale(code.lang).formatMessage(v)
    })
  ...
  static async export () {
    ...
    let csv = 'id,type,periodicity,step,sponsor,creation,start,end,duration,dispatchs,to_sent,turnover,email,gift,physical,styles,country,destination,address\n'

    for (const box of boxes) {
      const orders = await DB('order_box')
        .where('box_id', box.id)
        .where('is_paid', 1)
        .all()
  ...
  static async invoice (params) {
    ...
    const box = await DB('box')
      .where('id', order.box_id)
      .first()

    const customer = await DB('customer')
      .where('id', box.customer_id)
      .first()
  ...
  static async setPricesBox () {
    ...
    for (const box of boxes) {
      if (box.partner) {
        ...
        const cost = { ...costs[box.partner][box.shipping_type || 'standard'][box.type][box.periodicity] }
