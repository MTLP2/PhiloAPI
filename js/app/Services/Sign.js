...
Sign.loginFacebook = async (facebook) => {
  if (facebook.sponsor) {
...
Sign.checkSponsor = async (sponsor) => {
...
Sign.createUser = async (params) => {
  try {
    ...
    if (params.type === 'distributor' ||
    params.type === 'record_shop') {
...
Sign.createNotifications = (userId, newsletter) =>
  DB('notifications')
    .insert({
      user_id: userId,
...
Sign.confirmEmail = async (params) => {
  const user = await DB('user')
    .where('confirmation_code', params.code)
    .first()
...
Sign.forgotPassword = async (params) => {
  /**
...
Sign.resetPassword = async (params) => {
  const user = await DB('user')
    .where('token_password', params.code)
    .first()
...
Sign.checkReCaptcha = async (ip, captcha) =>
  new Promise(resolve => {
