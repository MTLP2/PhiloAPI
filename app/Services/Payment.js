...
Payment.editAddress = async (params) => {
  const payment = await DB('payment')
...
Payment.createInvoice = async (payment) => {
  let invoice = {}
...
Payment.confirmPay = async (payment, charge) => {
  payment.payment_id = charge.id
  payment.error = ''
  ...
  if (payment.order_shop_id) {
    const order = await DB('order_shop')
      .select('order_shop.transporter', 'shipping_type', 'address_pickup', 'order_shop.customer_id', 'user.email')
...
Payment.getCustomer = async (userId) => {
...
Payment.getCards = async (params) => {
  const user = await DB('user')
    .select('id', 'email', 'stripe_customer')
    .find(params.user.user_id)

  if (user.stripe_customer) {
    const customer = await Payment.getCustomer(params.user.user_id)
    customer.payment_methods = (await stripe.paymentMethods.list(
      {
        customer: process.env.NODE_ENV !== 'production' ? 'cus_KJiRI5dzm4Ll1C' : user.stripe_customer,
        // customer: user.stripe_customer,
        type: 'card'
      }
    )).data
    customer.default_source = customer.invoice_settings.default_payment_method || customer.default_source
    return customer
  } else {
    return []
  }
...
Payment.saveCards = async (params) => {
  const customer = await Payment.getCustomer(params.user.user_id)
...
Payment.getAccount = (id) =>
  stripe.accounts.retrieve(id)

Payment.getBalance = (id) =>
...
Payment.saveLagalEntity = async (params) => {
  let account = null
  ...
  if (customer.peronal_id_number) {
    p.legal_entity.peronal_id_number = customer.peronal_id_number ? customer.peronal_id_number : null
  }
  if (file) {
...
Payment.getBalances = async (date) => {
  const balance = {
    eur: 0,
    usd: 0,
    gbp: 0
...
Payment.getTransactions = async (date, start) => {
  const p = {
  ...
  for (let i = 0; i < payments.length; i++) {
    ...
    if (payment.payment_type === 'stripe') {
      if (!payment.transaction_id) {
        ...
        await DB('order').where('id', payment.id)
          .update({
