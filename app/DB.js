...
const DB = (tablee, idd) => {
  ...
  const db = {
    ...
    orWhereBetween (arg1, arg2) {
      ...
      p.query.orWhereBetween(arg1, arg2)
      return db
    },
    ...
    **/
    ...
    count () {
      return p.query.clone().select(DB.raw('count(*) AS total')).first().then(res => res.total)
    },

    sum (c) {
      return p.query.clone().select(DB.raw(`sum(${c}) AS total`)).first().then(res => res.total)
    },
      ...
      if (relation.type === 'belongsTo') {
        if (relation.keys.length === 0) {
          query.first(relation.columns)
            .where('id', this[relation.key])
        } else {
          query.select(relation.columns)
            .whereIn('id', relation.keys)
        }
