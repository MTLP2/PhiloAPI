...
MailJet.request = (endpoint, type = 'get', params = {}) => {
  return Utils
    .request(`${config.mailjet.url}${endpoint}`, {
      method: type,
      json: params,
      auth: {
...
MailJet.updateUsers = async () => {
  try {
    ...
    for (const user of users) {
      ...
      await DB('user').where('id', user.id)
        .update({
...
MailJet.setMailJetId = async () => {
  const users = await DB('user')
    .whereNull('mailjet_id')
    .all()
