...
Cart.getTaxRate = async (customer) => {
  ...
  if (params.boxes) {
    for (let i = 0; i < params.boxes.length; i++) {
      ...
      const box = await Box.calculate({
        country_id: params.boxes[i].gift && params.boxes[i].country_id
      ...
      if (params.boxes[i].shipping_type === 'pickup' && ['FR', 'BE', 'ES'].includes(params.boxes[i].country_id)) {
        cart.hasPickup = true
      }
    }
  }
  ...
  if (params.shops) {
    ...
    if (params.country_id) {
      ...
      if (params.shops.s_1_diggers) {
        ...
        items.push(
          ...params.shops.s_1_diggers.items
        )
...
Cart.calculateShop = async (p) => {
  ...
  await Promise.all(p.items.map(async (item, i) => {
    item.type = shop.type
...
Cart.getDiscountProject = (item, promo) => {
  if (promo.on_total) {
...
Cart.calculateShippingByTransporter = async (params) => {
  ...
  for (const transporter of transporters) {
    ...
    } else if (transporter[weight] && (!costs || !costs.standard || costs.standard > transporter[weight])) {
...
Cart.calculateShippingSoundmerch = async (params) => {
  const transporter = await DB('shipping_vinyl')
    .where('country_id', params.country_id === 'AU'
      ? params.country_id
      : '%')
...
Cart.calculateShipping = async (params) => {
  // add packaging
  params.weight += 340
  ...
  for (const ship of shippings) {
    if (params.is_shop && params.stock && (!params[`stock_${ship.transporter}`] || params[`stock_${ship.transporter}`] < params.quantity)) {
      continue
    }
    if (!ship.currency) {
      return { error: 'no_shipping' }
  ...
  res.pickup = shipping.pickup ? Utils.round(shipping.pickup / currencies[params.currency], 2) : null
  res.letter = shipping.letter ? Utils.round(shipping.letter / currencies[params.currency], 2) : null
  res.transporter = shipping.transporter

  return res
}

Cart.calculateItem = async (params) => {
  const p = params
  const res = {}
  ...
  if (p.project.only_country && !p.project.only_country.split(',').includes(params.customer.country_id)) {
    res.error = 'project_not_available_country'
  } else if (p.project.exclude_country && p.project.exclude_country.split(',').includes(params.customer.country_id)) {
    res.error = 'project_not_available_country'
  }

  if ((p.project.step !== 'in_progress' && p.project.step !== 'private') || p.project.error === 403) {
...
Cart.createOrder = async (params) => {
  ...
  if (calculate.shops) {
    await Promise.all(Object.keys(calculate.shops).map(async s => {
      ...
      await Promise.all(ss.items.map(async item => {
        const currencyRateProject = await Utils.getCurrencyComp(item.currency, item.currency_project)

        const i = await DB('order_item')
          .save({
            order_id: order.id,
  ...
  new Promise(async (resolve, reject) => {
    try {
      ...
      **/
      ...
      if ((params.boxes && params.boxes.some(b => b.monthly)) || (params.card_save && params.card.new)) {
        try {
      ...
      stripe.paymentIntents.create(intent, async (err, charge) => {
        try {
          ...
          if (charge.status === 'requires_action') {
            await DB('order').where('id', params.order.id)
              .update({
...
Cart.createPaypalPayment = (params) =>
...
Cart.validPayment = async (orderId, transactionId, status = 'confirmed') => {
  const order = await DB()
    .select('order.id', 'sub_total', 'total', 'shipping', 'order.currency', 'order.currency_rate', 'user_agent',
      'promo_code', 'discount', 'fee_bank', 'tax_rate', 'tax', 'payment_type', 'user.email', 'user_id')
    .from('order')
      ...
      await Promise.all(items.map(async item => {
        ...
        cio.track(user.id, {
          ...
          data: {
            ...
            picture: `${Env.get('STORAGE_URL')}/projects/${project.picture || project.id}/vinyl.png`,
  ...
  const items = []
  const t = t => Antl.forLocale('en').formatMessage(t)
...
Cart.related = async (cart) => {
  ...
  const res = await DB('project as p')
    .select('p.id', 'p.name', 'p.artist_name', 'vod.price', 'vod.currency', 'category', 'is_shop',
      'p.picture', 'slug', 'vod.user_id', 'vod.barcode', 'vod.type',
      DB.raw('vod.goal - vod.count - vod.count_other - vod.count_distrib as stock')
    )
    .join('vod', 'vod.project_id', 'p.id')
    .whereIn('p.id', [...items, ...accessories].map(p => p.id))
  ...
  for (const i in res) {
    res[i].prices = Utils.getPrices({ price: res[i].price, currencies, currency: res[i].currency })
  }
  return res
}
