...
Vod.save = async (params, pp) => {
  ...
  if (params.sponsor) {
    sponsor = await DB('sponsor')
      .where('code', params.sponsor)
  ...
  if (params.profile_name) {
    ...
    await DB('user').where('id', vod.user_id).update({
...
Vod.calculStock = async ({ id, isShop, quantity, transporter, recursive = true }) => {
  ...
  if (isShop) {
    p[`stock_${transporter}`] = p[`stock_${transporter}`] - quantity
    ...
    DB('vod_stock')
      .insert({
        project_id: id,
        type: transporter,
        old: p[`stock_${transporter}`] + quantity,
        new: p[`stock_${transporter}`],
        comment: 'order',
      ...
      if (recursive) {
        for (const barcode of p.barcode.split(',')) {
          ...
          for (const bundle of bundles) {
            Vod.calculStock({ id: bundle.project_id, recursive: false })
          }
        }
      }
    }
...
Vod.calculateVinyl = async (params) => {
  let size
  const user = await DB('user')
    .select('sponsor')
  ...
  if (params.format === '7"' || params.format === 'LP 7"' || params.format === 'EP 7"') {
    size = 7
  } else if (params.format === '10"' || params.format === 'LP 10"' || params.format === 'EP 10"') {
    size = 10
  } else if (params.format === '12"' || params.format === 'LP 12"' || params.format === 'EP 12"') {
    size = 12
  }
  ...
  for (let i = 1; i < 4; i++) {
    ...
    if (params.factory === 'sna' || params.factory === 'wolfpack' || params.factory === 'wolfpack2') {
    ...
    for (const cc of Object.keys(c)) {
      if (cc !== 'quantity') {
        if (params.fee) {
          ...
          c[cc] = Math.round(c[cc] * 1.30)
        } else {
          c[cc] = Math.round(c[cc] * 1.20)
        }
    ...
    const qtyDistrib = params.quantity_distribution > quantity2 ? quantity2 : params.quantity_distribution
    ...
    c.fee_distrib = Utils.round((params.price_distribution * qtyDistrib) * fee, 2) +
    ...
    c.profit_distribution = Utils.round((c.price * (quantity2 - qtyDistrib)) +
      (qtyDistrib * parseFloat(params.price_distribution)) - c.total_tax - c.fee_distrib, 0)
  ...
  const trans = {}
  trans.domestic_one = parseFloat(postage.domestic_one) +
    parseFloat((params.nb_vinyl - 1) * postage.domestic_plus)
  trans.domestic_plus = parseFloat(params.nb_vinyl * postage.domestic_plus)
  trans.ue_one = parseFloat(postage.ue_one) +
    parseFloat((params.nb_vinyl - 1) * postage.ue_plus)
  trans.ue_plus = parseFloat(params.nb_vinyl * postage.ue_plus)
  trans.world_one = parseFloat(postage.world_one) +
    parseFloat((params.nb_vinyl - 1) * postage.world_plus)
...
Vod.calculateWolfpack = (c, nb, quantity, params, allCosts) => {
  ...
  **/
  ...
  if (params.test_pressing) {
    c.test_pressing = allCosts.test_pressing_transport.all[quantity] +
...
Vod.calculateVdp = (c, nb, quantity, params, allCosts) => {
  const getValue = (type, sub) => {
    if (!allCosts[type]) {
      return 0
    }
...
Vod.calculateSqueezer = (c, nb, quantity, params, allCosts) => {
  const getValue = (type, sub) => {
    if (!allCosts[type]) {
      return 0
    }
...
Vod.calculateKuroneko = (c, nb, quantity, params, allCosts) => {
  const getValue = (type, sub) => {
    if (!allCosts[type]) {
      return 0
    }
...
Vod.changeStock = async (params) => {
  const vod = await DB('vod')
    .where('project_id', params.project_id)
    .first()

  const old = vod[`stock_${params.transporter}`]

  if (params.quantity) {
    vod[`stock_${params.transporter}`] = params.quantity
  } else if (params.add) {
    vod[`stock_${params.transporter}`] = vod[`stock_${params.transporter}`] + params.add
  } else if (params.sub) {
    vod[`stock_${params.transporter}`] = vod[`stock_${params.transporter}`] - params.sub
  }
  vod.updated_at = Utils.date()
  ...
  await DB('vod_stock')
    .insert({
      ...
      old: old,
      new: vod[`stock_${params.transporter}`],
