...
const translate = (key, lang = 'EN', payload) => {
  const wording = {
    ...
    confirmed_vod: lang === 'EN' ? ['Your payment has been successfully completed, thank you!', 'Your product is in the pre-order stage: you will be notified either when the project is launched or when it is cancelled. If the project is launched, the next steps are: factory pressing, logistic preparation, and delivery.', 'Thank you for your patience! 💪'] : ['Votre paiement a été réalisé avec succès, merci !', 'Votre produit est à l’étape de pré-commande : vous serez informé soit du lancement du projet soit de son annulation. Si le projet est lancé, les prochaines étapes sont : le pressage en usine, la préparation logistique, et la livraison.', 'Merci pour votre patience ! 💪'],
    confirmed_shop: lang === 'EN' ? ['Your payment has been successfully completed, thank you!', 'Your product is in immediate shipment: you will soon receive an email to confirm your address before delivery.', 'Thank you for your patience! 💪'] : ['Votre paiement a été réalisé avec succès, merci !', 'Votre produit est en expédition immédiate : vous recevrez bientôt un email pour confirmer votre adresse avant livraison.', 'Merci pour votre patience ! 💪'],
...
const generateOrderCard = async (order, lang, single = false) => {
  ...
  if (order.step !== 'launched') {
    ...
    if ((order.step === 'in_progress' || order.step === 'confirmed') && order.items.length === 1 && order.items[0].date_shipping) {
      ...
      const stepMessage = getMultiParagraph(order.step === 'confirmed' ? `confirmed_${order.type}` : order.step, lang)
