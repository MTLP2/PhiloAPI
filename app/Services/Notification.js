...
Notification.sendEmail = async (params) => {
...
Notification.template = (params) => {
  ...
  const t = v => Antl.forLocale(lang).formatMessage(v)
  ...
  try {
    ...
    params.items = `
      <table class="order">
        ${params.order_items && params.order_items.map(item =>
          `<tr>
            <td width="60"><img width="50" src="${item.picture}" alt="${item.name}" /></td>
            <td width="30" class="total">${item.quantity} x</td>
            <td width="100%">
              <a href="${Env.get('APP_URL')}/vinyl/${item.project_id}/${item.slug}"><b>${item.artist_name}</b><br/>${item.name}</a>
            </td>
          </tr>`
        )}
      </table>`
    ...
    if (params.boxes || params.order_items) {
      params.order = `
      <table class="order">
        ${params.boxes && params.boxes.map(item =>
          `<tr>
            <td width="60"><img width="50" src="https://storage.diggersfactory.com/assets/images/box/discovery_box.jpg" alt="${item.name}" /></td>
        ...
        ${params.order_items && params.order_items.map(item =>
          `<tr>
            ...
            <td width="100%">
              ...
              ${item.is_shop
                ? `${lang === 'fr'
                  ? 'Livraison: De 2 à 10 jours'
                  : 'Shipping : From 2 to 10 days'
                }`
                : ''
              }
              ${item.tracks
        ...
        ? `<tr>
          ...
          <td  class="total">
            ${Utils.round(params.order.sub_total - params.order.shipping + params.order.tax)} ${cur[params.order.currency]}
          </td>
        </tr>`
        : ''}
        <tr>
    ...
    `
      /**
      console.log(params)
      params.order = View.render('items', {
        items: params.items,
      ...
      <table align="center" style="color: #000; font-family: HelveticaNeue,helvetica,arial,sans-serif; margin: 0 auto;">
        <tr>
          <td class="header" style="text-align: center; padding: 20px;" align="center">
            <a href="${params.user.lang === 'fr' ? `${config.app.url}/fr` : config.app.url}" style="color: #76acc3;">
  ...
  if (params.user && params.user.name) {
    template += '<h1 style="font-size:25px; margin: 0; padding: 0; background: #ffd21e; border-radius: 5px 5px 0 0; padding: 30px; padding-bottom: 20px;">'
  ...
  if (params.lang === 'fr' || params.user.lang === 'fr') {
    if (!params.template) template += '<p style="margin-bottom: 0; color: #828585; ">Amicalement, <br>L’équipe Diggers Factory.</p>'
    template += `</div>
          </td>
        </tr>
        <tr>
    ...
    if (!params.template) template += '<p style="margin-bottom: 0; color: #828585; ">Sincerely,<br>Diggers Factory\'s Team.</p>'
    template += `
        </td>
      </tr>
      <tr>
