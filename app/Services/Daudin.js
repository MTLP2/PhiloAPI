...
class Daudin {
  ...
  static async getLines () {
    ...
    const manuals = await DB('order_manual')
      .select('order_manual.id', 'barcodes', 'address', 'customer.name as customer_name',
        'order_manual.email', 'order_manual.shipping_type', 'order_manual.address_pickup',
        'firstname', 'lastname', 'city', 'zip_code', 'state', 'customer.country_id', 'phone', 'ue')
    ...
    const boxes = await DB('box_dispatch')
      .select('box_dispatch.id', 'box.currency', 'box_dispatch.barcodes',
        'box_dispatch.is_daudin', 'user.email', 'shipping_type', 'address_pickup',
        'quantity', 'customer.name as customer_name', 'firstname', 'lastname', 'box.partner',
        'address', 'city', 'zip_code', 'state', 'customer.country_id', 'phone', 'ue',
  ...
  static async export () {
    ...
    await Notification.sendEmail({
      ...
      html: orders.length > 0
        ...
        Diggers Factory`,
      attachments: orders.length > 0 && process.env.NODE_ENV !== 'production'
        ? [
            {
              filename: `DF_${d}.zip`,
  ...
  static async csv (orders) {
    let csv = 'SOC;TITRE;VIDE;NO COMPTE;QTE;CODE;VIDE;VIDE;ADR 1;ADR 2;ADR 3;VIDE;VIDE;VIDE;NUM LIGNE;VIDE;CONTACT;NOMLIV1;NOMLIV2;CP;VILLE;PAYS;INSTLIV1;INSTLIV2;TELCLIENT;TRANSPORTEUR;VIDE;VIDE;VIDE;VIDE;VIDE;VIDE;CODEPAYS;MAIL;CODETRANSPORTEUR;VIDE;TYPE DE CDE'
    ...
    for (let i = 0; i < orders.length; i++) {
      ...
      csv += ';'
      csv += ';'
      csv += `${order.address && Utils.removeAccents(order.address).replace(/"/g, '\'').replace(/;/g, ',')};`
      csv += `${order.address2 || ''};`
      csv += ';'
      csv += ';'
      csv += ';'
  ...
  static async exportProject (params) {
    const lines2 = await DB('order_shop')
      .select('order_shop.id', 'quantity', 'barcode', 'address', 'customer.name',
        'firstname', 'lastname', 'city', 'zip_code', 'state', 'customer.country_id', 'phone', 'ue',
        'order_shop.tax_rate', 'address_pickup',
        'cat_number', 'order_item.price', 'order_item.total', 'order_item.currency',
        DB.raw('CONCAT(project.artist_name, \' - \', project.name) AS item'))
  ...
  static async parseTrackings (params) {
    ...
    worksheet.eachRow((row, rowNumber) => {
      ...
      if (tracking.code.search(' ') === -1 && tracking.code !== 'TRACKING' && tracking.code !== 'X' && tracking.code !== 'x') {
    ...
    for (const tracking of trackings) {
      if (tracking.id[0] === 'M') {
        await DB('order_manual')
          .where('id', tracking.id.substring(1))
          .update({
